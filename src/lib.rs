...
pub fn vbyte_encode<W: Write>(r: &[u64], target: &mut W) -> Result<usize, Error> {
    ...
    for i in r {
        ...
        total_size += target.write_all(&result[(10 - result[10]) as usize..10]).map(|()| result[10])? as usize;
    }
    Ok(total_size)
}
...
mod tests {
    ...
    fn test_heapless_vbyte_encode() {
        ...
        assert_eq!(VByteEncoded::new(std::u64::MAX).data,
    ...
    fn readme() {
        let mut buffer = vec![];
        let _len = vbyte_encode(vec![0, 130, 5, 65535, 0, 6, 2, 5, 4, 131].as_slice(), &mut buffer);
