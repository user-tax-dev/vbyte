//! This is a fork of https://github.com/CurrySoftware/perlin-core/tree/d60c665192e865c8f3e68f6ca28c6cfe361c0351 under MIT license.
//!
...
/// Performs a vbyte encode without allocating memory on the heap for a slice
/// Can then be written to a `Write`-implementor
pub fn vbyte_encode<W: Write>(r: &[u64], target: &mut W) -> Result<usize, Error> {
  let mut count;
  let mut result;
  ...
  for i in r {
    ...
    total_size += target
      .write_all(&result[(10 - result[10]) as usize..10])
      .map(|()| result[10])? as usize;
  }
  Ok(total_size)
}
...
impl VByteEncoded {
  ...
  /// Can then be written to a `Write`-implementor
  pub fn new(mut number: u64) -> Self {
    let mut count = 0;
  ...
  pub fn write_to<W: Write>(&self, target: &mut W) -> Result<u8, Error> {
    target
      .write_all(&self.data[(10 - self.bytes_used()) as usize..10])
      .map(|()| self.bytes_used())
  }
}
...
impl<R: Read> Read for VByteDecoder<R> {
  fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
    ...
    if self.filled > 0 {
      ...
      bytes_read += (&self.buf[..self.filled as usize]).read(buf)?;
...
impl<R: Read> Iterator for VByteDecoder<R> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    let read = self
      .source
      .read(&mut self.buf[self.filled as usize..])
      .unwrap();
    let mut ptr = 0;
...
mod tests {
  ...
  fn test_heapless_vbyte_encode() {
    ...
    assert_eq!(
      VByteEncoded::new(std::u64::MAX).data,
  ...
  fn test_vbyte_decode() {
    assert_eq!(
      VByteDecoder::new(vec![0x80].as_slice()).collect::<Vec<_>>(),
      vec![0]
    );
    assert_eq!(
      VByteDecoder::new(vec![0x85].as_slice()).collect::<Vec<_>>(),
      vec![5]
    );
    assert_eq!(
      VByteDecoder::new(vec![0xFF].as_slice()).collect::<Vec<_>>(),
      vec![127]
    );
    assert_eq!(
      VByteDecoder::new(vec![0x80, 0x81].as_slice()).collect::<Vec<_>>(),
      vec![0, 1]
    );
    assert_eq!(
      VByteDecoder::new(vec![0x03, 0x7F, 0xFF, 0x01, 0x82, 0x85].as_slice()).collect::<Vec<_>>(),
      vec![0xFFFF, 130, 5]
    );
    assert_eq!(
      VByteDecoder::new(vec![0x80].as_slice()).collect::<Vec<_>>(),
  ...
  fn overflowing() {
    assert_eq!(
      VByteDecoder::new(vec![0x81; 255].as_slice()).collect::<Vec<_>>(),
      vec![1; 255]
    );
  }
  ...
  fn more_data() {
    let data = vec![
      0x80, 0x01, 0x82, 0x85, 0x03, 0x7F, 0xFF, 0x80, 0x86, 0x82, 0x85, 0x84, 0x01, 0x83,
    ];
    ...
    let mut buffer = vec![];
    let len = vbyte_encode(
      vec![0, 130, 5, 65535, 0, 6, 2, 5, 4, 131].as_slice(),
      &mut buffer,
    );
    assert_eq!(buffer, data);
    assert_eq!(len.ok(), Some(14));
  }
  ...
  fn readme() {
    ...
    assert_eq!(
      buffer,
      vec![0x80, 0x01, 0x82, 0x85, 0x03, 0x7F, 0xFF, 0x80, 0x86, 0x82, 0x85, 0x84, 0x01, 0x83]
    );

    let decoder = VByteDecoder::new(buffer.as_slice());
    assert_eq!(
      decoder.collect::<Vec<u64>>(),
      vec![0, 130, 5, 65535, 0, 6, 2, 5, 4, 131]
    );
  }
  ...
  fn edge_cases() {
    // 0
    assert_eq!(
      VByteDecoder::new(VByteEncoded::new(0).data_buf()).collect::<Vec<_>>(),
      vec![0]
    );

    // MAX
    assert_eq!(
      VByteDecoder::new(VByteEncoded::new(std::u64::MAX).data_buf()).collect::<Vec<u64>>(),
      vec![std::u64::MAX]
    );

    // too many bytes = corrupted data
    assert_eq!(
      VByteDecoder::new(vec![127, 127, 127, 127, 127, 127, 127, 127, 127, 127, 255].as_slice())
        .collect::<Vec<_>>(),
      vec![]
    );
    // MAX + n = corrupted data
    assert_eq!(
      VByteDecoder::new(vec![2, 127, 127, 127, 127, 127, 127, 127, 127, 255].as_slice())
        .collect::<Vec<_>>(),
      vec![]
    );
    // zero-bytes
    assert_eq!(
      VByteDecoder::new(vec![0; 100].as_slice()).collect::<Vec<_>>(),
      vec![]
    );
  }
}
